        ...
        for row in data:
            if row[1] == sys.argv[i]:
                for j in range(2, len(row)):
                    ...
                    CalculatedData.append((row[0], 1958 + j, int(row[j]) / 1000000))
    return demography
...
def fit1(data, total: bool):
    print("Fit 1")
    if (total):
        for j in range (1, len(sys.argv)):
            for k in range(len(data)):
                if (sys.argv[j] == data[k][1]):
                    print(data[k][1])
                    tmp_data = sum_data([data[k]])
                    m, n = get_function_fit1(tmp_data)
                    resu = 0
                    for i in range(len(tmp_data)):
                        func = (1960 + i) * m + n
                        resu += (pow(func - tmp_data[i], 2) / len(tmp_data))
                    CalculatedData.append((data[k][0], 2050, resu))
